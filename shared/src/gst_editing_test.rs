...
pub fn main_loop(uri: &str) -> Result<(), glib::BoolError> {
    ...
    let project_loc = format!("file:///{}/test-project.xges", projects_location());
    // let project = ges::Project::new(None);

    let timeline: ges::Timeline = ges::Timeline::new_audio_video();

    let clip_location = uri;
    ...
    let loc = format!("file:///{}/test-timeline.xges", projects_location());
    ...
    let clip = ges::UriClipAsset::request_sync(loc.as_str()).unwrap();
    ...
    layer
        .add_asset(
            &clip,
            Some(gst::ClockTime::from_seconds(20)),
            None,
            None,
            TrackType::UNKNOWN,
        )
    ...
    let loc = format!("file:///{}/test-timeline2.xges", projects_location());
...

    {
        ...
        pipeline
            .set_render_settings(
                "file:////home/ajsmith/cs310/server/application_data/out.webm",
                &container_profile,
            )
            .unwrap();
    ...
    return Ok(());
    let project_location = format!("{}/test-project", projects_location());
