...
export class SourceClip {
    ...
    constructor(id: ID, name: string, file_location: string) {
        this.id = id;
        this.name = name;
    ...
    static deserialise(obj: any) {
        if (Utils.propsUndefined(obj.id, obj.name, obj.file_location)) {
            throw new Error("Could not deserialise! Malformed object");
        }
        return new SourceClip(obj.id, obj.name, obj.file_location);
    }
}
