  ...
  loop {
    ...
    match rx.try_recv() {
      ...
      Err(mpsc::TryRecvError::Empty) => {
        let connection_status = x.connection_status.clone();
        ...
        match connection_status {
          ...
          _ => {
            ...
            if !locked_state.file_written {
              println!("Saving new state to file...");

              if let Some(store) = &locked_state.store {
                let json_string = serde_json::to_string(store).unwrap();
                let composited_clips = store.clips.composited.clone();
                    ...
                    match networking::receive_message(&mut stream) {
                      Ok(Message::NewChunk) => {
                        ...
                        if segment_number == 0 {
                          ...
                          let (video_stream_count, audio_stream_count) =
                            (video_streams.len(), audio_streams.len());
